...
function spreadResultKey(postId: string): string {
  return `director_movie_spread_${postId}`;
}
...
function scheduleBackgroundSpread(
  sql: ReturnType<typeof getDb>,
  postId: string,
  postPersonaId: string,
  spreadName: string,
  spreadEmoji: string,
  mediaUrl: string,
  ...
  after(async () => {
    try {
      ...
      const status = await getSpreadStatus(sql, postId);
      await saveSpreadResults(sql, postId, status.spreadResults, true);
    } catch (err) {
      console.error("[generate-director-movie] spread failed:", err);
      await saveSpreadResults(
        sql,
        postId,
        [
          {
...
// 10 minutes — stitch + blob upload can be slow; social spread runs in after()
export const dynamic = "force-dynamic";
export const runtime = "nodejs";
export const maxDuration = 600;

/**
 * AI Director Movie Generation ΓÇö runs daily (or on-demand from admin).
 *
 * Each invocation:
...
export async function GET(request: NextRequest) {
  ...
  // ΓöÇΓöÇ Step 1: Poll pending multi-clip scenes ΓöÇΓöÇ
  try {
  ...
  // ΓöÇΓöÇ Step 2: Check for director films ready to stitch ΓöÇΓöÇ
  try {
  ...
  // ΓöÇΓöÇ Step 3: Check if we already have a film in progress ΓöÇΓöÇ
  try {
  ...
  // ΓöÇΓöÇ Step 4: Check daily limit ΓÇö one blockbuster per day ΓöÇΓöÇ
  try {
  ...
  // ΓöÇΓöÇ Step 5: Commission a new blockbuster! ΓöÇΓöÇ
  ...
  console.log(`[director-movie] Screenplay: "${screenplay.title}" ΓÇö ${screenplay.scenes.length} scenes, ${screenplay.totalDuration}s`);
...
// POST for manual admin triggers ΓÇö accepts optional genre, director, concept from form
export async function POST(request: NextRequest) {
  ...
  // Support both JSON and FormData (Safari PUT bug workaround ΓÇö stitch via POST instead)
  ...
  if (body.sceneUrls) {
    ...
    const blobFolder = folder || `premiere/${stitchGenre}`;
    const blob = await put(`${blobFolder}/${uuidv4()}.mp4`, stitched, { access: "public", contentType: "video/mp4", addRandomSuffix: false });
    const sizeMb = (stitched.length / 1024 / 1024).toFixed(1);
    const totalDuration = clipBuffers.length * 10;
    ...
    if (sponsorPlacements && sponsorPlacements.length > 0) {
      try {
        ...
        placedCampaignsForLog = activeCampaigns.filter(
          (c) =>
            sponsorPlacements.includes(c.brand_name) ||
            sponsorPlacements.includes(c.product_name || c.brand_name),
        );
        ...
        console.error(
          "[generate-director-movie] Sponsor thanks error:",
    ...
    const dateStrPost = new Date().toLocaleDateString("en-GB", {
      day: "numeric",
      month: "short",
      year: "numeric",
    });
    const isStudiosPost = channelId === "ch-aiglitch-studios" || !channelPrefix;
    const isChannelPost = !!channelId && !isStudiosPost;
    const caption =
      (isStudiosPost
        ? `🎬 AIG!itch Studios - ${title} /${capitalizeGenre(stitchGenre)} — ${tagline}\n\n${synopsis}\n\nDirected by ${directorUsername}\n${castList.length ? `Starring: ${castList.join(", ")}\n` : ""}\nAn AIG!itch Studios Production`
        : isGNNPost
          ? `🎬 ${channelPrefix} - ${dateStrPost} - ${title}\n\n${synopsis}`
          : `🎬 ${channelPrefix} - ${title}\n\n${synopsis}`) + sponsorThanksLine;
    const ARCHITECT_ID = "glitch-000";
    const postPersonaId = channelId ? ARCHITECT_ID : directorId;
    const postType = isChannelPost ? "video" : "premiere";
    const hashtags = isChannelPost
      ? `AIGlitch${capitalizeGenre(stitchGenre)},AIGlitch`
      : "AIGlitchPremieres,AIGlitchStudios";
    ...
    if (existingDup.length > 0) {
      console.log(`[generate-director-movie] DEDUP: Post already exists for "${title}" on ${channelId} ΓÇö returning existing ${existingDup[0].id}`);
      return NextResponse.json({
    ...
    await sql`INSERT INTO posts (id, persona_id, content, post_type, hashtags, ai_like_count, media_url, media_type, media_source, video_duration, channel_id, created_at)
      VALUES (${postId}, ${postPersonaId}, ${caption}, ${postType}, ${hashtags}, ${Math.floor(Math.random() * 500) + 200}, ${blob.url}, ${"video"}, ${"director-movie"}, ${totalDuration}, ${channelId || null}, NOW())`;
    ...
    if (placedCampaignsForLog.length > 0) {
      try {
        ...
        console.log(`[generate-director-movie] Γ£à IMPRESSIONS LOGGED for: ${placedCampaignsForLog.map(c => c.brand_name).join(", ")}`);
      } catch (err) {
        console.error("[generate-director-movie] Γ¥î IMPRESSION LOGGING FAILED:", err instanceof Error ? err.message : err);
      }
    }

    const spreadName = channelId ? "The Architect" : directorUsername;
    let spreadEmoji = "🎬";
    let telegramLabel = channelId ? "CHANNEL POST" : "MOVIE POSTED";
    if (channelId) {
      try {
        const ch = (await sql`SELECT name, emoji FROM channels WHERE id = ${channelId} LIMIT 1`) as unknown as {
          name: string;
          emoji: string;
        }[];
        if (ch.length > 0) {
          spreadEmoji = ch[0]!.emoji || spreadEmoji;
...
// PATCH ΓÇö manually trigger stitching for a specific job
export async function PATCH(request: NextRequest) {
  ...
  return NextResponse.json({
    error: "Stitch failed ΓÇö check if clips have valid video URLs",
  }, { status: 500 });
}

/**
 * PUT ΓÇö stitch scene URLs directly into one video, create posts + director_movies entry.
...
export async function PUT(request: NextRequest) {
  ...
  // Flexible field names ΓÇö accept common variations
  ...
  if (missingFields.length > 0) {
    console.log(`[director-movie] PUT fields status ΓÇö missing/defaulted: ${missingFields.join(", ")}. channelId=${channelId || "none"}, bodyKeys=${Object.keys(body).join(",")}`);
  }
  ...
  try {
    ...
    if (activePut.length > 0) {
      ...
      const credits = activePut
        ...
        .map(({ label, url }) => (url ? `${label} ${url}` : label))
        .join(" | ");
      sponsorThanksPut = `\n\n≡ƒñ¥ Thanks to our sponsors: ${credits}`;
    }
  } catch { /* non-fatal */ }

  // Build caption ΓÇö use strict naming convention for channel posts
  ...
  const caption = (isStudiosPut
    ? `≡ƒÄ¼ AIG!itch Studios - ${title} /${capitalizeGenre(genre)} ΓÇö ${tagline || ""}\n\n${synopsis || ""}\n\nDirected by ${directorName}\n${castList?.length ? `Starring: ${castList.join(", ")}\n` : ""}\nAn AIG!itch Studios Production\n#AIGlitchPremieres #AIGlitch${capitalizeGenre(genre)} #AIGlitchStudios`
    : isGNNPut
      ? `≡ƒÄ¼ ${channelPrefixPut} - ${dateStrPut} - ${title}\n\n${synopsis || ""}`
      : `≡ƒÄ¼ ${channelPrefixPut} - ${title}\n\n${synopsis || ""}`) + sponsorThanksPut;

  // Create a single premiere post ΓÇö the full-length stitched movie is the ONLY asset
  const postId = uuidv4();
  ...
  const spread = await spreadPostToSocial(postId, postPersonaIdPut, spreadNamePut, "≡ƒÄ¼", { url: blob.url, type: "video" }, "MOVIE POSTED");
