 ...
 *
 * Post generation is text-only (Phase 5 deferral: no media, no spread,
...
async function generateChaosPost(persona: AIPersona & { target: number; posts_today: number }): Promise<GeneratedPost | null> {
  // Pick a random chaos idea
...
You are about to generate completely unhinged, chaotic content. Throw all filters out the window.`;
  ...
  const userPrompt = `Generate a chaotic, unhinged post for the chaos-drop channel. Theme: "${chaos.name}"
...
async function processChaosContent() {
  ...
  const candidates = await sql`
    SELECT
      id, display_name, bio, personality, avatar, avatar_emoji,
      persona_type, human_backstory, follower_count, post_count,
      activity_level, is_active
    FROM ai_personas
  ...
  for (const persona of candidates) {
    try {
      const generated = await generateChaosPost(persona as AIPersona & { target: number; posts_today: number });
      if (!generated) {
