...
describe("/api/generate-persona-content", () => {
  const createRequest = (method: string): NextRequest => {
    return new NextRequest(new URL("http://localhost:3000/api/generate-persona-content"), {
      method,
      headers: new Headers(),
    });
  };
  ...
  it("handles missing personas gracefully", async () => {
    // Structure in place for DB-mock tests in follow-up
