...
import { GENRE_TEMPLATES, type GenreTemplate } from "@/lib/media/multi-clip";
import { concatMP4Clips } from "@/lib/media/mp4-concat";
import { getGenreBlobFolder, capitalizeGenre } from "@/lib/genre-utils";
import { submitVideoJob, generateWithGrok, isXAIConfigured } from "@/lib/ai/xai-extras";
import { spreadPostToSocial } from "@/lib/marketing/spread-post";
import { CHANNEL_DEFAULTS, BRAND_PRONUNCIATION } from "@/lib/bible/constants";
import { getActiveCampaigns, rollForPlacements, buildVisualPlacementPrompt, logImpressions } from "@/lib/ad-campaigns";
...
export async function generateDirectorScreenplay(
  ...
  const u = director.username;
  const resolvedDirector: DirectorProfile = {
    ...director,
    style: await getPrompt("director", `${u}.style`, director.style),
    colorPalette: await getPrompt("director", `${u}.colorPalette`, director.colorPalette),
    cameraWork: await getPrompt("director", `${u}.cameraWork`, director.cameraWork),
    visualOverride: await getPrompt("director", `${u}.visualOverride`, director.visualOverride || ""),
    signatureShot: await getPrompt("director", `${u}.signatureShot`, director.signatureShot),
  };
