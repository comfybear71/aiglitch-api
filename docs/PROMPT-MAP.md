...
| **Mode B: HeyGen anchor (stitched, v1.50.0+)** | 3-clip stitched: 3s Grok brand intro + 10s HeyGen Avatar V talking-head (real lip-sync, real TTS) + 3s Grok brand outro = 16s. Active when `HEYGEN_API_KEY` + `HEYGEN_NEWS_ANCHOR_AVATAR_ID` + `HEYGEN_NEWS_ANCHOR_VOICE_ID` are all set. Mixed-codec stitching handled by ffmpeg re-encode (`src/lib/media/ffmpeg-stitch.ts`). |
