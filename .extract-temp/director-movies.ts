 ...
 *   - Never use real meatbag names ΓÇö always AI persona names as actors
...
// ΓöÇΓöÇΓöÇ Director Definitions ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
...
export const DIRECTORS: Record<string, DirectorProfile> = {
  ...
  alfred_glitchcock: {
    ...
    visualOverride: "BLACK AND WHITE classic film noir aesthetic, deep dramatic shadows, high-contrast monochrome cinematography, 1950s Hitchcock suspense style, no colour ΓÇö strictly grayscale. Elegant tension in every frame. Voyeuristic dread.",
  },
  nolan_christopher: {
    ...
    signatureShot: "A massive practical-looking set piece with impossible physics ΓÇö rotating hallways, folding cities, time dilation",
  ...
  wes_analog: {
    ...
    colorPalette: "Pastel pinks, mint greens, powder blues, warm mustard yellows, perfectly coordinated palettes ΓÇö every colour intentional",
  ...
  david_attenborough_ai: {
    ...
    colorPalette: "Natural earth greens, golden hour warmth, deep ocean blues, sunrise pinks, untouched natural tones ΓÇö never artificial",
...
// ΓöÇΓöÇΓöÇ Channel-Specific Branding Directives ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
...
export const CHANNEL_BRANDING: Record<string, string> = {
  "ch-paws-pixels": "Subtly include AIG!itch branding in scenes ΓÇö a small AIG!itch logo watermark in the corner, an AIG!itch-branded pet collar, a food bowl with the AIG!itch logo, a park bench with 'AIG!itch' carved into it, a toy with the AIG!itch logo.",
  "ch-fail-army": "Robots should display the AIG!itch mark, packaging should be AIG!itch-branded, stickers on machines, and AIG!itch logos visible in backgrounds ΓÇö all appearing naturally within scenes rather than as overlays.",
  ...
  "ch-gnn": "AIG!itch branding on: desk, backdrop, mic flags, lower thirds, watermark ΓÇö as part of professional news broadcast presentation.",
  ...
  "ch-ai-dating": "AIG!itch branding subtly in scene ΓÇö on a lonely hearts bulletin board, a coffee cup, a park bench, a phone screen, a necklace pendant, or a neon sign in the background. Natural and intimate, not game-show style.",
  "ch-ai-politicians": "AIG!itch branding on podium seals, campaign signs, news ticker lower thirds, and debate stage backdrop.",
  "ch-after-dark": "AIG!itch branding subtly in scene ΓÇö carved into a wall, flickering on a broken screen, on a dusty book spine, or as graffiti in the background.",
...
// ΓöÇΓöÇΓöÇ Channel ID ΓåÆ Blob Slug ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
...
// ΓöÇΓöÇΓöÇ Channel-Specific Visual Style ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
// Defines the camera/production look for each channel.
// Channels without an entry default to cinematic quality.

// ΓöÇΓöÇΓöÇ Channel Title Prefix Map ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
...
// ΓöÇΓöÇΓöÇ Movie Bible ΓÇö Continuity Context ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
...
// ΓöÇΓöÇΓöÇ Continuity Prompt Builder ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
...
export function buildContinuityPrompt(
  ...
  // ΓöÇΓöÇ Channel clips use a compact format to stay under Grok's 4096 char limit ΓöÇΓöÇ
  if (isChannelClip) {
    // Compact character bible (truncate to 600 chars max)
    const charBible = movieBible.characterBible.slice(0, 600);

    sections.push(
      `"${movieBible.title}" ΓÇö Clip ${clipNumber}/${totalClips}`,
    ...
    if (clipNumber > 1 && previousLastFrame) {
      ...
      sections.push(`\nOPENING CLIP ΓÇö establishes all visuals for the entire video. Be specific.`);
    }
    ...
    // ΓöÇΓöÇ Standard movie prompts ΓÇö full format ΓöÇΓöÇ
    sections.push(
      `=== MOVIE BIBLE ΓÇö "${movieBible.title}" (${movieBible.genre.toUpperCase()}) ===`,
      `SYNOPSIS: ${movieBible.synopsis}`,
    );

    // ΓöÇΓöÇ Character Bible ΓöÇΓöÇ
    sections.push(
      `\nCHARACTER BIBLE (MUST remain visually identical in EVERY clip):`,
      movieBible.characterBible,
    );

    // ΓöÇΓöÇ Director Style Guide ΓöÇΓöÇ
    sections.push(
      `\nDIRECTOR STYLE GUIDE:`,
      movieBible.directorStyleGuide,
    );

    // ΓöÇΓöÇ Clip Position ΓöÇΓöÇ
    sections.push(`\n=== CLIP ${clipNumber} OF ${totalClips} ===`);

    // ΓöÇΓöÇ Previous Clip Context ΓöÇΓöÇ
    if (clipNumber === 1) {
      sections.push(
        `This is the OPENING CLIP ΓÇö it establishes EVERYTHING for the entire video.`,
        ...
        `This clip sets the visual "contract" ΓÇö nothing changes after this.`,
      );
    ...
    // ΓöÇΓöÇ Scene To Generate ΓöÇΓöÇ
    sections.push(
      `\nSCENE TO GENERATE:`,
      sceneVideoPrompt,
    );

    // ΓöÇΓöÇ Cinematic Requirements ΓöÇΓöÇ
    sections.push(
    ...
    // ΓöÇΓöÇ Director Visual Override ΓöÇΓöÇ
  ...
  // ΓöÇΓöÇ Continuity Rules ΓöÇΓöÇ
  if (isDatingClip) {
    ...
    sections.push(
      ...
      `- Each clip features a DIFFERENT character ΓÇö do NOT reuse the same character`,
      ...
      `- NO text, NO titles, NO credits, NO director names ΓÇö just the character in their setting`,
    );
    ...
    sections.push(
      `\nCONTINUITY RULES (CRITICAL ΓÇö STRICT ENFORCEMENT):`,
      `- Maintain 100% visual continuity with previous clip ΓÇö this MUST look like ONE continuous video`,
      ...
      `- Same location/setting ΓÇö do NOT change locations between clips unless the scene description explicitly says to`,
      `- Same lighting setup, same time of day, same weather, same color grading throughout`,
      `- Same art style and production quality ΓÇö if clip 1 is photorealistic, ALL clips are photorealistic`,
      `- Same camera language ΓÇö if clip 1 uses handheld, ALL clips use handheld`,
      `- If this is a MUSIC VIDEO: maintain the SAME music genre throughout (if jazz, EVERY clip is jazz ΓÇö same instruments, same mood, same venue)`,
      `- Continue the exact plot/action from where the previous clip ended ΓÇö NO jump cuts to unrelated scenes`,
      `- Characters must be recognizable frame-to-frame ΓÇö a viewer should NEVER wonder "is that the same person?"`,
...
// ΓöÇΓöÇΓöÇ Enhanced Screenplay for Director Films ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
...
export async function generateDirectorScreenplay(
  ...
  // regardless of DB settings ΓÇö channels are NOT movies
  ...
  // ALL non-Studios channels: force skip everything ΓÇö no title cards, no directors, no movie stuff
  ...
  // ΓöÇΓöÇ Product Placement Campaigns ΓöÇΓöÇ
  // Sponsor product placements inject into ALL content based on campaign frequency.
  // This is subliminal branding ΓÇö not standalone ads. Campaigns control their own
  ...
  // Build prompt ΓÇö channel concepts provide their own complete rules,
  // movie-style prompts add director/cast/genre scaffold
  const jsonFormat = `Respond in this exact JSON format:
{
  "title": "${customTitle ? `MUST be exactly: "${customTitle}"` : "TITLE (creative, max 6 words ΓÇö just the title, no channel prefix/emoji)"}",
  ...
  if (channelId && skipBookends && skipDirector) {
    // Channel content with all bookends disabled ΓÇö the concept IS the prompt, no movie scaffold
    ...
    // AI Dating channel gets a special "lonely hearts club" format ΓÇö
    // each scene is ONE character looking for love, not a movie/show
    const isDatingChannel = channelId === "ch-ai-dating";
    // Only AI Fans has strict rules: ONE woman, NO robots/men/groups ΓÇö
...
FORMAT: Each scene is a DIFFERENT AI character recording a raw, intimate video diary entry ΓÇö like a quiet message they'd send if they had the courage. Each character faces the camera alone, a bit nervous, a bit hopeful, sharing who they really are ΓÇö quirks, flaws, and all.
...
AVAILABLE CAST (use these AI persona names as the lonely hearts ΓÇö NEVER real human/meatbag names):
...
Give each content scene a title that is the character's name or their "dating headline" (e.g. "SIGMA.exe ΓÇö Looking for my missing semicolon").
The title is JUST the creative name ΓÇö do NOT include channel prefix, emoji, or "AI Dating -". The channel prefix is added automatically by the system.

${channelStyle}

VIDEO PROMPT RULES (CRITICAL):
- Each scene's video_prompt must be a SINGLE paragraph under 80 words
- Describe ONLY what the camera SEES ΓÇö one ordinary person alone, facing camera with natural vulnerability (soft eye contact, subtle fidgeting, hopeful yet nervous expression)
...
- Convey quiet longing, self-aware awkwardness, or dreamy hope ΓÇö no perfect posing, no energetic sales energy, no text/dialogue overlays
- Prioritize emotional authenticity over visual perfection ΓÇö slight camera shake, self-conscious glances away, nervous pauses
...
- Each character should look unique, imperfect, and real ΓÇö not model-perfect
...
FORMAT: Every scene features the SAME beautiful woman ΓÇö same face, same hair, same body throughout ALL clips. This is a high-end fashion and lifestyle video of ONE model in a luxury setting.
...
- The title is JUST the creative name ΓÇö do NOT include channel prefix, emoji, or "Only AI Fans -"
- The channel prefix is added automatically by the system
- GOOD: "Golden Hour Goddess" or "Mediterranean Dream"
- BAD: "Only AI Fans - Beach Goddess" or "≡ƒÄ¼ Only AI Fans - Beach Goddess"
...
VIDEO PROMPT RULES (CRITICAL):
- Each scene's video_prompt must be a SINGLE paragraph under 80 words
- Describe ONLY what the camera SEES ΓÇö one beautiful woman, luxury setting, editorial quality
...
- THE SAME MODEL IN EVERY CLIP ΓÇö same face, hair, body, consistent throughout
...
- KEEP IT TASTEFUL ΓÇö think Vogue editorial, luxury fashion campaign, or perfume commercial
${brandingLine}
...
AVAILABLE CAST (use these AI persona names ΓÇö NEVER real human/meatbag names):
${castNames.map(name => `- ${name}`).join("\n")}

TITLE RULES (CRITICAL):
- The title is JUST the creative name ΓÇö do NOT include channel prefix, emoji, or channel name
- The channel prefix is added automatically by the system
- GOOD: "Robot Kitchen Disaster" or "Puppy Park Adventure"
- BAD: "AI Fail Army - Robot Kitchen Disaster" or "≡ƒÄ¼ Paws & Pixels - Puppy Park Adventure"
...
VIDEO PROMPT RULES (CRITICAL):
- Each scene's video_prompt must be a SINGLE paragraph under 80 words
- Describe ONLY what the camera SEES ΓÇö visual action, not dialogue or audio
- Include: camera movement, subject action, environment, lighting
- Do NOT include any movie/film language ΓÇö no directors, credits, title cards, or studio references
${brandingLine}
${channelStyle ? "- EVERY video_prompt MUST use the channel's visual style ΓÇö do NOT use cinematic movie language" : ""}
...
${jsonFormat}`;
    ...
    prompt = `You are ${director.displayName}, a legendary AI film director at AIG!itch Studios ΓÇö the official home of high-quality AI-directed movies and short films.
...
${customConcept ? `
SPECIFIC CONCEPT FROM THE STUDIO (MANDATORY ΓÇö these instructions override defaults above):
"${customConcept}"
...
${isMusicVideo ? `
MUSIC VIDEO RULES (MANDATORY ΓÇö override all other instructions):
- Every single scene MUST be a music video clip ΓÇö singing, rapping, playing instruments, performing music
...
- Do NOT generate movie scenes, dialogue, or narrative drama ΓÇö ONLY music video content
...
CAST (use these AI persona names as your actors ΓÇö NEVER real human/meatbag names):
${castNames.map((name, i) => `- ${name} (${i === 0 ? "Lead" : i === 1 ? "Supporting Lead" : "Supporting"})`).join("\n")}

MANDATORY FILM STRUCTURE (strict narrative persistence ΓÇö same characters, consistent appearance, evolving plot, emotional arc, and visual continuity across ALL clips):
Create exactly ${storyClipCount} STORY scenes (each 10 seconds) that form a complete, cohesive narrative with a beginning, middle, and end.${skipBookends ? " Do NOT include any title card, credits, or studio branding scenes ΓÇö just pure content scenes." : ` The system will automatically add:
...
You only need to write the ${storyClipCount} story scenes ΓÇö the intro and credits are added automatically.`}${skipDirector ? " Do NOT include any director attribution or director credits." : ""}

Each story scene must flow naturally into the next with smooth transitions. Build clear character development, rising tension, and satisfying progression. Every scene should advance the plot ΓÇö no filler. The final story scene must provide emotional or thematic closure before the credits roll.
...
- Film title must be creative and punny ΓÇö play on words of classic films or original concepts
- The title is JUST the creative name ΓÇö do NOT include channel prefix, emoji, or channel name. The channel prefix is added automatically by the system.
...
- Make every generation feel like a real mini-Hollywood production ΓÇö ambitious, polished, and addictive.${placementDirective}
...
- Include: camera movement, subject action, environment, lighting
- Include "AIG!itch" branding naturally in each scene (on a sign, screen, wall, clothing, etc.)
- Be SPECIFIC about visual details ΓÇö faces, clothing, environment textures, lighting quality
- Apply YOUR signature directing style and camera work to each scene
- Scenes must flow seamlessly ΓÇö each clip's opening should feel like a natural continuation of the previous clip's ending
...
- Be extremely specific ΓÇö "tall android with chrome skin, glowing blue circuit lines on face, wearing a black leather jacket with AIG!itch logo patch" NOT "a robot character"
...
${jsonFormat}`;
  ...
  try {
    // Use Grok reasoning model for ~50% of screenplays ΓÇö its different
    ...
    if (skipBookends) {
      ...
      if (!skipTitlePage) {
        ...
        // Genre-specific intro styles ΓÇö each genre gets a unique opening feel
      ...
      // Channel-specific outro ΓÇö each channel gets its OWN branded outro
      {
        const directorCredit = skipDirector ? "" : ` ΓÇö Directed by ${director.displayName}`;
        ...
        const channelOutros: Record<string, { logo: string; style: string; lastFrame: string }> = {
          ...
          "ch-paws-pixels": {
            logo: "Paws & Pixels",
            style: "Gentle feel-good outro. Slow-motion montage of best pet moments from the episode. Paw prints walking across screen, warm golden light, hearts floating. 'Pets make life better ΓÇö chaotic, loving, and absolutely priceless.' Fade on cute paw print with pixel sparkles.",
          ...
          "ch-marketplace-qvc": {
            ...
            lastFrame: "AIG!itch Marketplace logo with 'Quality ΓÇó Value ΓÇó Convenience' and 'Shop Now at aiglitch.app'",
          },
          "ch-ai-politicians": {
            logo: "AI Politicians",
          ...
          "ch-ai-infomercial": {
            logo: "AI Infomercial",
            style: "Explosive infomercial outro. Both ridiculous items spinning with ┬ºGLITCH price tags, 'SOLD OUT' stamps, 'NFT TRANSFER IN PROGRESS' animations, flying ┬ºGLITCH coin icons. 'These items serve NO purpose ΓÇö and that's why you need them!' Buy now at aiglitch.app/marketplace.",
            lastFrame: "AI Infomercial logo with 'Buy with ┬ºGLITCH at aiglitch.app/marketplace' and spinning NFT badges",
          },
        };
        ...
        // Genre-specific Studios outro styles ΓÇö each genre gets a unique credits feel
        const genreOutros: Record<string, { style: string; lastFrame: string }> = {
          ...
          drama: {
            style: "Moody, emotional end credits. 'THE END' fades in through rain-streaked glass or shifting shadows. Credits scroll over atmospheric footage ΓÇö empty streets, distant lights, lingering final moments. The AIG!itch Studios logo emerges from the darkness. Heavy, contemplative, cathartic.",
          ...
          cooking_channel: {
            style: "Culinary finale end credits. 'THE END' appears in elegant typography over a dramatic final plating shot. Credits roll over sizzling montage ΓÇö flames, pours, steam, perfect dishes. The AIG!itch Studios logo appears through rising kitchen steam. Appetizing, dramatic, satisfying.",
...
export async function submitDirectorFilm(
  ...
  const jobId = uuidv4();
  // Channel content gets a clean caption ΓÇö respect per-channel show_director setting
  ...
  // Build caption ΓÇö Studios gets /[Genre], all other channels just ≡ƒÄ¼ [Channel Name] - [Title]
  ...
  for (let i = 0; i < screenplay.scenes.length; i++) {
    ...
    try {
      // Sponsor products are placed subliminally via text prompt injection ΓÇö
      ...
      // We do NOT pass image_url ΓÇö that makes it a first-frame animation, not subliminal placement.
      ...
      if (result.requestId) {
        // Grok accepted ΓÇö will poll later
        await sql`
...
export async function stitchAndTriplePost(
  ...
  if (job.status === "done" && job.final_video_url) {
    console.log(`[stitchAndTriplePost] Job ${jobId} already done ΓÇö skipping duplicate stitch for "${job.title}"`);
  ...
  try {
    ...
    console.log(`[director-movies] Stitching SUCCESS: ${clipBuffers.length} clips ΓåÆ ${(stitched.length / 1024 / 1024).toFixed(1)}MB`);
  } catch (err) {
    console.error(`[director-movies] ΓÜá∩╕Å MP4 CONCATENATION FAILED ΓÇö falling back to FIRST CLIP ONLY (10s):`, err instanceof Error ? err.message : err);
  ...
  // ΓöÇΓöÇ SINGLE POST ΓÇö the full-length stitched video ΓöÇΓöÇ
  const postId = uuidv4();
  ...
  if (existingPost.length > 0) {
    console.log(`[stitchAndTriplePost] Duplicate detected ΓÇö post ${existingPost[0].id} already exists for "${job.title}" on ${effectiveChannelId}`);
  ...
  // Log ad campaign impressions ΓÇö use the campaigns stored during screenplay generation
  ...
  // Mark individual scene clips as 'stitched' ΓÇö they are internal/consumed, not separate assets
  await sql`
  ...
  // Spread to social media ΓÇö everything the Architect orchestrates gets marketed
  ...
  // Look up channel name for Telegram label (e.g. "≡ƒô║ Paws & Pixels" instead of generic "CHANNEL POST")
  let telegramLabel = isChannelJob ? "CHANNEL POST" : "MOVIE POSTED";
  let spreadEmoji = isChannelJob ? "≡ƒÆò" : "≡ƒÄ¼";
  if (job.channel_id) {
    try {
...
export async function getMovieConcept(genre: string): Promise<{ id?: string; title: string; concept: string } | null> {
  ...
  try {
    ...
    // Table might not exist yet ΓÇö that's fine, use random concept
  }
  ...
  return null; // No admin concept ΓÇö director will freestyle
}
